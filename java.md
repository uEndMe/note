...
* 结构分析
    ...
    * 存储结构 // 元素数据及元素关系。基于计算机实现的逻辑结构
        * 线性
        * 链式
        * 索引
        * 散列
        
    * 运算结构 // 基于存储结构，实现逻辑结构的步骤
        * 分配资源
        * 插入删除
        * 查询遍历
        * 修改排序
...
索引结构：一组节点及索引表，索引表存索引及节点地址
  * 优点：使用索引定位节点，检索快
  * 缺点：需要额外空间存索引表，增删节点重建索引耗时多
  
散列结构：根据元素的哈希值，取模得到存储地址
  * 优点：检索，增删快速
  * 缺点：无序，需要更多空间，哈希值不能重复
```

> ```
> * 随机访问：根据下标及连续的地址，通过计算地址的偏移量实现
> ```
...
```
基于逻辑结构实现的线性集合，不对存储结构做要求
```

```
* 逻辑结构：线性结构
* 存储结构：允许不连续的存储空间
* 存储特点：由节点组成，节点数量动态，存储数据及下个地址指针
```
...
```
基于逻辑结构的单向单口线性集合，限制集合方向及入口
```

```
* 逻辑结构：先进后出，适用于多重嵌套的逻辑结构 // 仅顶层元素可见
```
...
```
基于逻辑结构的单向线性集合，限制集合方向
```

```
* 逻辑结构：先进先出，适用于排队的逻辑 // 仅俩端可见，且单方向
* 存储结构：可以是数组，链表
```
...
11  12  21  22
  ...
  * 前序：根左右 // 0,1,11,12,2,21,22
  * 中序：左根右 // 11,1,12,0,21,2,22
  * 后序：左右中 // 11,12,1,21,22,2,0
```
...
```
// 双向链表原理
1. 实例化时，声明属性，first,last为null
2. add时，新增node节点，重新连接上下地址。
```
...
// 哈希表保存元素原理
1. 使用数组+链表的方式存储元素
2. 实例化时，初始化容量 initialCapecity  为16
3. 当元素数量到达 负载因子 loadfactor 比例时，进行双倍扩容
4. 扩容后，需要重新计算地址，将元素重新分布
5. jdk1.8开始，单独链表超过8时，数组小于64提前扩充，否则链表改红黑树。
```

> ```
> // 哈希表保存元素原理 - 详情
> 1. 使用数组+链表存储数据，初始长度16
> 2. 当元素数超过0.75倍时，进行2倍扩容
> 3. 扩容后，重新计算分布
> 4. jdk1.8开始，
>   1. 单独链表的元素超过8个，且数组长度小于64，提前扩容
>   2. 单独链表超过8，且数组达到64，将链表改红黑树。
> ```

> ```
> * initial Capacity：16  // 初始容量 // 因内沈，可怕习体
> * load Factor：0.75     // 负载因子 // load 发可偷
> ```

> ```
> 扩容是耗性能操作，推荐在实例化时给出预期容量
> ```
>
> ```
> // 源码笔记
...
* 无序性（非随机）
* 不可重复性
* 自定类去重，需重写 hashCode 和 equals
```

```
// 哈希去重原理
1. 调用 hashCode 获取下标放入数组
2. 下标重复，调用 equals 判断
3. 相等，不执行本次写入操作
4. 不相等，链接到元素后边
```
...
[参考文章-知乎](https://zhuanlan.zhihu.com/p/95156642)
